...
class OrderBook:
    ...
    def __init__(self, dt, D, L, mt=0, lower_bound=-1,
                 upper_bound=1, Nx=100, diffusion_scheme='implicit'):
        """
    ...
    def update_prices(self):
        ...
        self.price = (- self.best_ask * self.best_ask_density
                      + self.best_bid *
                      self.best_bid_density)/(-self.best_ask_density
                                              + self.best_bid_density)

    def execute_metaorder(self):
        """ Execute at current time the quantity dq = mt * dt at the best price
        which depends on the sign of mt.
        If mt > 0, then the metaorder is a buy and hence ask orders are
        executed, first at price best_ask.
        If mt < 0, then the metaorder is a sell and hence bid orders are
        executed, first at price best_bid.
