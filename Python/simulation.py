...
class Simulation:
    ...
    def __init__(self, T, Nt, book_args, metaorder_args, model_type='discrete'):
        ...
        self.A_low = abs(self.m0)/(self.book.L*np.sqrt(self.book.D * np.pi))
        self.A_high = np.sqrt(2)*np.sqrt(abs(self.m0)/self.book.L)
        self.participation_rate = abs(self.m0)/self.J if self.J !=0 else float("inf")
    ...
    def plot_err(self, ax, relative=False, symlog=False):
        ...
        Keyword Arguments:
            ...
            symlog {bool} -- Use symlog scale (default: {False})
        """
        self.growth_error_abs = self.prices[self.n_start:self.n_end] - self.growth_th
